...
window.addEventListener('load', async () => {
  ...
  if (savedSession && savedPhone) {
    ...
    console.log('Session ID:', savedSession);
    console.log('Phone:', savedPhone);
    
    // Проверяем валидность сессии
    try {
      const testRes = await fetch(`${API_URL}/api/users?sessionId=${encodeURIComponent(savedSession)}`);
      
      if (testRes.ok) {
        console.log('Сессия валидна, восстановление...');
        ...
        console.warn('Сессия устарела, очищаем cookies');
    ...
    root.innerHTML = `
      <div class="container">
        ...
        <input 
          type="text" 
          id="phoneInput" 
          placeholder="+375000 или любой номер"
          value="${state.phoneNumber}"
        >
        <button onclick="checkPhone()">Продолжить</button>
        <div class="hint">Введите любой номер телефона - новые пользователи создаются автоматически!</div>
      </div>
    `;
...
function renderUsersList() {
  ...
  return users.map(user => {
    const isNew = user.isNewContact;
    const avatar = user.username ? user.username[0].toUpperCase() : (user.phoneNumber.slice(-3) || '?');
    const displayName = user.username || user.phoneNumber;
    const badge = isNew ? '<span class="new-badge">Новый</span>' : '';
    
    return `
      <div class="chat-item" onclick="selectChat('${user.phoneNumber}')">
        <div class="avatar ${isNew ? 'avatar-new' : ''}">${avatar}</div>
        <div class="chat-info">
          <div class="chat-name">
            ${displayName}
            ${badge}
          </div>
          <div class="chat-preview" style="color: #a0aec0;">
            ${isNew ? 'Начать чат с этим номером' : user.phoneNumber}
          </div>
        </div>
      </div>
    `;
...
async function checkPhone() {
  ...
  try {
    ...
    if (data.registered) {
      ...
      state.error = 'Ошибка при проверке номера';
    }
  } catch (err) {
...
async function initMessenger() {
  ...
  pollInterval = setInterval(() => {
    if (state.selectedChat && state.sessionId) {
...
async function loadAllUsers() {
  if (!state.sessionId) {
    console.error('No session ID for loading users');
    return;
  }
  
  try {
    console.log('Loading all users with session:', state.sessionId);
    const res = await fetch(`${API_URL}/api/users?sessionId=${encodeURIComponent(state.sessionId)}`);
...
async function searchUsers(query) {
  if (!state.sessionId) {
    console.error('No session ID for search');
    return;
  }
  
  if (!query.trim()) {
    console.log('Empty query, loading all users');
    loadAllUsers();
    return;
  }
  ...
  try {
    ...
    const url = `${API_URL}/api/users/search?sessionId=${encodeURIComponent(state.sessionId)}&query=${encodeURIComponent(query)}`;
    ...
    if (!res.ok) {
      ...
      if (res.status === 401) {
        console.error('Session invalid during search');
        logout();
        return;
      }
